...
| Donar | Botón | `<WEB_PUBLIC_URL>/donate` |
| GitHub | Texto | Fijo en el código: `InfoLinks.GitHubUrl` |
| Web | Texto | `WEB_PUBLIC_URL` |

- **`WEB_PUBLIC_URL` no es `WEB_URL`.** `WEB_URL` es la dirección desde la que el bot llama a la web (en el ejemplo, `127.0.0.1`, con `INTERNAL_API_KEY` en cada petición): un usuario no puede abrirla. Por eso los enlaces públicos usan su propia variable.
...
- **En producción, al arrancar, se avisa por log** de las variables que faltan (`[info] SUPPORT_INVITE_URL, WEB_PUBLIC_URL not set or invalid...`). En `canary` y `developing` no se avisa, porque ahí es normal no tenerlas.
