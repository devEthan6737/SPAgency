...
- **Entradas simultáneas**: `isJoinBurst` (`JoinBurstWindowMs` = 10s, `JoinBurstMinCount` = 3), sobre un [`RollingWindowCounter`](../src/systems/shared/RollingWindowCounter.ts) compartido — deliberadamente **no** `BurstTracker` (ver `antiraid.md`), porque `BurstTracker.hit()` dispara una vez y resetea el contador, lo que solo marcaría a *una* cuenta de toda una oleada de entradas simultáneas en vez de a todas. Este contador es el que originó `RollingWindowCounter` como pieza reutilizable — la primera versión de esta señal lo traía hecho a mano, y se extrajo en cuanto `AutomodSystem` necesitó lo mismo (ver `moderation.md`).
