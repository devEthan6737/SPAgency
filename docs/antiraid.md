...
Todo lo que sigue existe como columna en `guild_protection` pero no tiene ninguna implementación en Seyfert todavía: `antitokens`, `antijoins`, `maliciousMemberAction`/`warnEntry` (diseñado del todo, ver [`malicious-members.md`](malicious-members.md)), `verification`, `purgeWebhooksAttacks`, `intelligentSOS`, `intelligentAntiflood`/`antiflood`, `bloqEntritiesByName`, `bloqNewCreatedUsers`, `raidmode`. La mayoría vive junta en el evento `guildMemberAdd` del bot legacy y está pendiente de una sesión de diseño propia, igual que tuvo `AntiraidSystem` (`antibots` ya está hecho, ver [`antibots.md`](antibots.md)).
