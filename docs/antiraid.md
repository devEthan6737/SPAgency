...
Todo lo que sigue existe como columna en `guild_protection` pero no tiene ninguna implementación en Seyfert todavía: `antitokens`, `antijoins`, `verification`, `purgeWebhooksAttacks`, `intelligentSOS`, `intelligentAntiflood`/`antiflood`, `bloqEntritiesByName`, `bloqNewCreatedUsers`, `raidmode`. La mayoría vive junta en el evento `guildMemberAdd` del bot legacy y está pendiente de una sesión de diseño propia, igual que tuvo `AntiraidSystem` (`antibots` y `maliciousMemberAction` ya están hechos, ver [`antibots.md`](antibots.md) y [`malicious-members.md`](malicious-members.md)).
