 ...
 * declarative way to express triggers) that does `pg_notify('guild_config_changed', NEW.id)`. A
 * separate trigger function from `guild_protection`/`guild_configuration`'s, since this table's
 * primary key is `id`, not `guild_id`. `GuildConfigCache` listens on the same channel either way —
